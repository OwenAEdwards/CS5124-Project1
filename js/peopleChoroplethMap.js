export default class PeopleChoroplethMap {
  ...
  initVis() {
    ...
    const populationValues = vis.data.objects.counties.geometries
      .map((d) => d.properties.pop)
      .filter((d) => d !== null && d !== undefined);

    // Color scale for population data
    vis.colorScale = d3
      .scaleLinear()
      .domain(d3.extent(populationValues)) // Set domain from min to max population
    ...
    vis.counties = vis.g
      ...
      .attr("fill", (d) => {
        ...
        return countyPop != null ? vis.colorScale(countyPop) : "url(#lightstripe)"; // Striped pattern for missing data
      })
      .attr("class", (d) => (d.properties.pop ? "active" : "inactive")); // Grayscale inactive counties
      ...
      .on("mousemove", (event, d) => {
        ...
        tooltip
          ...
          .html(`
            ...
            <div><strong>Total Population:</strong> ${countyPop ? countyPop.toLocaleString() : "No data available"}</div>
          `);
      })
      .on("mouseleave", () => {
